        ...
        const agentMetrics = agents.map((agent) => {
          ...
          const agentMessages = messages.filter(
            (m: {
              ...
              sender_type: string;
              ai_metadata?: { response_time_ms?: number };
              metadata?: { sentiment?: { sentiment: string } };
            }) => {
              const conv = conversations.find(
                (c: { _id: string; agent_id: string }) =>
                  String(c._id) === m.conversation_id &&
                  c.agent_id === agent.id,
              );
          ...
          const messagesWithResponseTime = agentMessages.filter(
            (m: { ai_metadata?: { response_time_ms?: number } }) =>
              m.ai_metadata?.response_time_ms,
          );
