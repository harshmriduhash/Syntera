/**
 * Next.js API Route - Mark Notification as Read
...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(`${AGENT_SERVICE_URL}/api/notifications/${id}/read`, {
      method: 'PATCH',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to mark notification as read' }, { status: response.status })
    }
